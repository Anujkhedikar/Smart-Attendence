...
const todaysLogs = [
  {
    id: 1,
    name: 'Sarah Johnson',
    avatar: 'https://i.pravatar.cc/150?u=sarah', // Using placeholder images
...
const AdminDashboard: React.FC = () => {
  ...
  const [isAttendanceMenuOpen, setIsAttendanceMenuOpen] = useState(true); // Open by default for this view
    ...
    <div className="flex min-h-screen bg-gray-50 font-sans">
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col shadow-sm z-10 fixed h-full">
        ...
        <nav className="flex-1 p-4 overflow-y-auto">
          <div className="space-y-1 mb-6">
            ...
            <Link to="/admin-dashboard" className="flex items-center space-x-3 px-3 py-2.5 rounded-lg bg-indigo-50 text-indigo-600 font-medium">
      ...
      <main className="flex-1 ml-64 bg-gray-50">
        ...
        <div className="px-8 py-8 max-w-7xl mx-auto">
          ...
          {/* Page Title */}
          <div className="mb-8">
             <h3 className="text-xl font-bold text-gray-900">Employee Attendance Overview</h3>
             <p className="text-gray-500 text-sm mt-1">Real-time attendance tracking for today, Oct 26, 2023.</p>
          </div>

          {/* Stats Cards Row */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            {adminStats.map((stat, index) => (
              <div key={index} className={`rounded-xl p-6 shadow-sm border border-gray-100 flex items-center justify-between ${stat.bgColor}`}>
                <div>
                  <h4 className={`text-sm font-medium mb-1 ${stat.textColor}`}>{stat.title}</h4>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <tr key={log.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <img className="w-8 h-8 rounded-full mr-3" src={log.avatar} alt={log.name} />
                          <span className="text-sm font-medium text-gray-900">{log.name}</span>
                        </div>
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500">{log.department}</td>
                      <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500">{log.checkIn}</td>
                      <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500">{log.checkOut}</td>
                      <td className="px-4 py-4 whitespace-nowrap text-center">
                        <span className={`px-2.5 py-1 text-xs font-medium rounded-full ${log.statusColor}`}>
                          {log.status === 'Present' && '● '}
                          {log.status === 'Absent' && '● '}
                          {log.status === 'Half Day' && '● '}
                          {log.status === 'On Leave' && '● '}
            ...
            <div className="flex items-center justify-between mt-6 border-t border-gray-100 pt-4">
                <p className="text-sm text-gray-500">Showing 5 of 142 employees</p>
                <div className="flex space-x-2">
                    <button className="px-3 py-1 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50" disabled>Previous</button>
